...
if __name__ == "__main__":
    ...
    parser.add_argument("--inference_type",
                    type=str,
                    default='test',
                    help="The image set for inference : test or test-generated")

    args = parser.parse_args()
    inference_type = args.inference_type
    if(inference_type == 'test'):
        print("TEST")
    if(inference_type == 'test-generated'):
    ...
    if NR_CONCEPTS == 8374:
        concept_file = os.path.join(BASE_DIR, "concepts.csv")
        print("Using concept File : ", concept_file)
        df_all_concepts = pd.read_csv(concept_file, sep=",")
    else:
        concept_file = os.path.join(BASE_DIR, f'concepts_top_{NR_CONCEPTS}.txt')
        print("Using concept File : ", concept_file)
        df_all_concepts = pd.read_csv(os.path.join(BASE_DIR, f'concepts_top_{NR_CONCEPTS}.txt'), sep=",")
    ...
    test_data = ImageDataset(
        args.images_csv, df_all_concepts=concept_file, transform=transform, sub_dataset='test',
        image_dir = args.image_dir #"/scratch/rm5707/roco-dataset-master/data/test/radiology/images"
    )
    ...
    for counter, data in enumerate(tqdm(test_loader)):
        ...
        pred    = torch.tensor(np.expand_dims(np.where(outputs.numpy()[0] >= 0.5, 1, 0), axis=0))
    ...
    if('test' in args.images_csv):
        evaluation_df.to_csv(os.path.join(LOGDIR, f"preds_{inference_type}.txt"), sep=",", index=False, header=True)
        print("Created : ", os.path.join(LOGDIR, f"preds_{inference_type}.txt"))
    else:
    ...
    print("F1 score (weighted):", f1_score(y_true=y_true, y_pred=y_pred, average='weighted'))
    print("F1 score (None)    :", f1_score(y_true=y_true, y_pred=y_pred, average=None))
    print("F1 score (micro)    :", f1_score(y_true=y_true, y_pred=y_pred, average='micro'))
